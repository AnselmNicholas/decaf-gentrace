...
void get_new_access(xed_operand_action_enum_t curr_access, xed_operand_action_enum_t *old_access, xed_operand_action_enum_t *new_access) {
	switch (curr_access) {
...
static void update_operand_contents(OperandVal *opnd) {
	if (opnd->type == TRegister) {
		...
		DECAF_read_mem(cpu_single_env, opnd->addr, (int) (opnd->length), (uint8_t *) &(opnd->value));
	}
}
...
void update_written_operands(EntryHeader *eh) {
	...
	while ((eh->operand[i].type != TNone) && (i < MAX_NUM_OPERANDS)) {
		...
		case XED_OPERAND_ACTION_CRW: {
			...
			memcpy((void *) &(eh->operand[first_empty_idx]), (void *) &(eh->operand[i]), sizeof(OperandVal));
...
void include_written_operands(EntryHeader *eh) {
	...
	while ((eh->operand[i].type != TNone) && (i < MAX_NUM_OPERANDS)) {
		...
		case XED_OPERAND_ACTION_RCW:
		case XED_OPERAND_ACTION_CRW: {
			// Copy operand to post op

			pop_idx++;
			eh->num_poperands++;
			memcpy((void *) &(eh->poperand[pop_idx]), (void *) &(eh->operand[i]), sizeof(OperandVal));

			// Update operand access for post operands
			get_new_access(eh->operand[i].access, &old_access, &new_access);
			eh->poperand[pop_idx].access = new_access;

			// Update value for new operand
			update_operand_contents(&(eh->poperand[pop_idx]));
